...
No UPDATE in place, no DELETE on versioned artifacts. Every mutation creates a new
versioned record. Claude is the sole writer; the owner observes via read-only UI.

RETENTION POLICY: Versioned artifacts (specifications, procedures) are never deleted.
Telemetry records (assertion_runs) may be pruned to prevent unbounded growth —
see the SessionStart hook for details.

This is the reference implementation. Start with these 4 core tables and add more
as your project grows. See MEMBASE-4-CLAUDE.md for the full pattern with extended
tables (tests, test_plans, work_items, documents, etc.).
...
def _row_to_dict(row: sqlite3.Row) -> dict[str, Any]:
    """Convert a Row to dict, auto-parsing JSON fields."""
    d = dict(row)
...
class KnowledgeDB:
    ...
    Core tables: specifications, operational_procedures, assertion_runs,
    session_prompts. See MEMBASE-4-CLAUDE.md for extended tables.
    """
    ...
    def insert_spec(
        ...
        *,
        ...
        """Insert a new specification (creates version 1, or next version if ID exists)."""
        ...
        conn.execute(
            """INSERT INTO specifications
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (id, version, title, description, spec_type, status,
    ...
    def update_spec(
        ...
        **fields: Any,
        ...
        Uses _UNSET sentinel internally so callers can pass None or [] to clear fields.
        Fields not provided keep their current value.
        """
        ...
        # Tags and assertions: allow explicit None/[] to clear
        _UNSET = self._UNSET
    ...
    def insert_op_procedure(
        ...
        *,
        ...
        """Insert a new operational procedure."""
        ...
        conn.execute(
            """INSERT INTO operational_procedures
               (id, version, title, description, type, trigger, scope, steps,
                changed_by, changed_at, change_reason)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (id, version, title, description, type, trigger, scope,
    ...
    def prune_assertion_runs(self, keep: int = 5) -> int:
        """Prune old assertion_runs, keeping only the latest N per spec.

        DESIGN NOTE: The never-delete principle applies to VERSIONED ARTIFACTS
        (specifications, procedures) where history is the point. Assertion runs
        are TELEMETRY — high-volume, machine-generated records where only the
        recent trend matters. Pruning telemetry is consistent with the principle
        because:
        1. Assertion results are derived data (re-runnable from specs + codebase)
        2. Without pruning, assertion_runs grows 10x faster than all other tables
        3. The latest N runs per spec provide sufficient trend data
        """
        conn = self._get_conn()
        before = conn.execute("SELECT COUNT(*) FROM assertion_runs").fetchone()[0]
        conn.execute(f"""
            DELETE FROM assertion_runs WHERE rowid NOT IN (
                SELECT rowid FROM (
                    SELECT rowid, ROW_NUMBER() OVER (
                        PARTITION BY spec_id ORDER BY rowid DESC
                    ) AS rn FROM assertion_runs
                ) WHERE rn <= {keep}
            )
        """)
        conn.commit()
    ...
    # ------------------------------------------------------------------
    # Session Prompts (handoff system)
    # ------------------------------------------------------------------
    ...
    def insert_session_prompt(
        ...
        Args:
            session_id: The session that generated this prompt (e.g., "S42").
            prompt_text: The full prompt text for the next session.
            context: Optional structured context (version, test counts, next tasks).
        """
    ...
    def consume_session_prompt(self, session_id: str) -> None:
        """Record consumption of a session prompt (append-only)."""
    ...
    def export_json(self, output_path: str | Path | None = None) -> str:
        """Export the entire database as JSON (full append-only backup)."""
        conn = self._get_conn()
        tables = ["specifications", "operational_procedures",
